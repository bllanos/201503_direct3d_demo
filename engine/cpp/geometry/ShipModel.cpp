...
#define SHIPMODEL_SOUTH  XMFLOAT3(0.0f, -1.0f, 0.0f)
#define SHIPMODEL_NORTH  XMFLOAT3(0.0f, 1.0f, 0.0f)
#define SHIPMODEL_CENTRE XMFLOAT3(0.0f, 0.0f, 0.0f)

ShipModel::ShipModel(const bool enableLogging, const std::wstring& msgPrefix,
	Config* sharedConfig) :
...
HRESULT ShipModel::configure(const std::wstring& scope, const std::wstring* configUserScope, const std::wstring* logUserScope) {
	...
	// Geometry properties
	size_t nColumns = SHIPMODEL_COLUMNS_DEFAULT;
	size_t nRows = SHIPMODEL_ROWS_DEFAULT;
	m_debugWinding = SHIPMODEL_DEBUG_FLAG_DEFAULT;

	if (hasConfigToUse()) {

		// Configure base members
		std::wstring logUserScopeDefault(SHIPMODEL_LOGUSER_SCOPE);
		std::wstring configUserScopeDefault(SHIPMODEL_CONFIGUSER_SCOPE);
		...
		if (FAILED(SkinnedTexturedGeometry::configure(scope, configUserScope, logUserScope))) {
			...
			if (retrieve<Config::DataType::INT, int>(scope, SHIPMODEL_COLUMNS_FIELD, intValue)) {
				nColumns = *intValue;
			}

			if (retrieve<Config::DataType::INT, int>(scope, SHIPMODEL_ROWS_FIELD, intValue)) {
				nRows = *intValue;
			}

			if (retrieve<Config::DataType::BOOL, bool>(scope, SHIPMODEL_DEBUG_FLAG_FIELD, boolValue)) {
	...
	if (nColumns < SHIPMODEL_COLUMNS_MIN) {
		m_nColumns = SHIPMODEL_COLUMNS_MIN;
		logMessage(L"Input number of columns was less than "
			+ std::to_wstring(SHIPMODEL_COLUMNS_MIN)
	...
	if (nRows < SHIPMODEL_ROWS_MIN) {
		m_nRows = SHIPMODEL_ROWS_MIN;
		logMessage(L"Input number of rows was less than "
			+ std::to_wstring(SHIPMODEL_ROWS_MIN)
...
HRESULT ShipModel::initialize(ID3D11Device* const device,
	...
	else if (bones->size() != SHIPMODEL_NBONES) {
	...
	if (FAILED(result)) {
		...
		if (bindMatrices == 0) {
			ownBindMatrices = new DirectX::XMFLOAT4X4[SHIPMODEL_NBONES];
			DirectX::XMFLOAT3* cornerPositions = new DirectX::XMFLOAT3[SHIPMODEL_NBONES];
			cornerPositions[0] = SHIPMODEL_CENTRE;
			cornerPositions[1] = SHIPMODEL_SOUTH;
			cornerPositions[2] = SHIPMODEL_NORTH;

			for (size_t i = 0; i < SHIPMODEL_NBONES; ++i) {
...
HRESULT ShipModel::addIndexedVertices(
	...
	vertex[bottomCenterIndex].position = SHIPMODEL_SOUTH;
	vertex[bottomCenterIndex].normal = SHIPMODEL_SOUTH;
	...
	vertex[topCenterIndex].position = SHIPMODEL_NORTH;
	vertex[topCenterIndex].normal = SHIPMODEL_NORTH;
...
HRESULT ShipModel::uvToPosition(DirectX::XMFLOAT3& position, const float u, const float v) const {
...
HRESULT ShipModel::uvToIndex(DirectX::XMFLOAT4& color, const float u, const float v) const {
	if (u < 0.0f || u > 1.0f || v < 0.0f || v > 1.0f) {
		return MAKE_HRESULT(SEVERITY_ERROR, FACILITY_BL_ENGINE, ERROR_INVALID_INPUT);
	}
	color.x = u;
...
HRESULT ShipModel::uvToNormal(DirectX::XMFLOAT3& normal, const float u, const float v) const {
	return uvToPosition(normal, u, v); // Assuming a unit sphere
}

HRESULT ShipModel::uvToBoneIDs(unsigned int boneIDs[4], const float u, const float v) const {
	if (u < 0.0f || u > 1.0f || v < 0.0f || v > 1.0f) {
		return MAKE_HRESULT(SEVERITY_ERROR, FACILITY_BL_ENGINE, ERROR_INVALID_INPUT);
	}
	boneIDs[0] = 0; // Centre
...
HRESULT ShipModel::uvToBoneWeights(DirectX::XMFLOAT4& boneWeights, const float u, const float v) const {
	if (u < 0.0f || u > 1.0f || v < 0.0f || v > 1.0f) {
		return MAKE_HRESULT(SEVERITY_ERROR, FACILITY_BL_ENGINE, ERROR_INVALID_INPUT);
	}
