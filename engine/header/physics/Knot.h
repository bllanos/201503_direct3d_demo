...
protected:
	/* A Knot can be created single or double-sided.
	   This will determine which points
	   are output by getControlPoints().

	   If 'controlPoints' is not null, it will be used
	   to set the initial control point values.
	 ...
	 */
	Knot(const PointSet side, const DirectX::XMFLOAT3* const controlPoints = 0);
...
public:
	 ...
	 */
	virtual HRESULT getControlPoints(DirectX::XMFLOAT4* const controlPoints, const PointSet side);
