...
HRESULT AlbedoTextureTestState::initialize(ID3D11Device* device, int screenWidth, int screenHeight) {
	...
	bool cornerFixed[ALBEDOTEXTURETESTSTATE_NQUADBONES];
	cornerFixed[0] = true;
	cornerFixed[1] = false;
	cornerFixed[2] = true;
	cornerFixed[3] = true;

	bool cornerOrbit[ALBEDOTEXTURETESTSTATE_NQUADBONES];
	cornerOrbit[0] = true;
	cornerOrbit[1] = true;
	cornerOrbit[2] = false;
	cornerOrbit[3] = false;

	XMFLOAT3 cornerAxes[ALBEDOTEXTURETESTSTATE_NQUADBONES];
